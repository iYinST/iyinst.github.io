\---[          Repositories     5 ](https://github.com/iYinST?tab=repositories)
...
class CART:
    def __init__(self, objective='reg', max_depth=10, min_samples_leaf=2, min_impurity_decrease=0.):
        self.objective = objective
        if self.objective == 'reg':
            self.loss = mse_loss
            self.leaf_weight = np.mean
        elif self.objective == 'classify':
            self.loss = gini_loss
    ...
    def fit(self, X, y):
        ...
        self.generate_node(self.root, X, y, self.depth)
        pass

    def generate_node(self, root: Node, X: np.array, y: np.array, depth: int):
        ...
        for feature_index in range(X.shape[1]):
            ...
            for candidate in split_candidate:
                ...
                # 计算分裂后的loss
                split_loss = (len(left) * self.loss(y[left]) + len(right) * self.loss(y[right])) / len(y)
        ...
        if split_point == -1:
            ...
            self.generate_node(root.left, X, y, depth + 1)
            self.generate_node(root.right, X, y, depth + 1)
...
mse_loss和gini_loss的计算如下。

```python
def mse_loss(target: np.array, pred = None):
    if pred:
...
```o
28.781810208088906 0.0029997825622558594
26.035714466239682 4.325863838195801
1.0 1.0 1.0 0.003008127212524414
0.7 0.7 0.7 0.18848323822021484
```
